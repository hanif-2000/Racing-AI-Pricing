...
function HistoryTab() {
  ...
  const fetchHistory = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await API.history.getHistory(daysFilter);
      if (data.success) {
  ...
  const getMonthStats = () => {
    const monthMeetings = historyData.filter((m) => {
      ...
      return (
        meetDate.getMonth() === currentMonth.getMonth() &&
  ...
  const getDayStatus = (day) => {
    ...
    return meetings.every((m) => m.status === "completed")
      ? "completed"
      : "partial";
  };
                  ...
                  <div className="meeting-info">
                    ...
                    <span
                      style={{
                        ...
                        color:
                          meeting.type === "jockey" ? "#4ade80" : "#c084fc",
                        padding: "4px 10px",
                        borderRadius: "6px",
                        fontSize: "0.75rem",
                        fontWeight: "600",
